...
export const AntigravitySettings = makeProviderSettingsSchema(
  {
    ...
    binaryPath: makeBinaryPathSetting("agy").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description: "Path to the Google Antigravity (agy) CLI.",
        providerSettingsForm: { placeholder: "agy", clearWhenEmpty: "omit" },
      }),
    ),
    modelOverride: TrimmedString.pipe(
      Schema.withDecodingDefault(Effect.succeed("")),
      Schema.annotateKey({
        title: "Model override",
        description:
          "Optional model name to pass to agy (e.g. gemini-3.5-flash). If blank, defaults to agy's configured model.",
        providerSettingsForm: {
  ...
  {
    order: ["binaryPath", "modelOverride"],
  },
);
...
const AntigravitySettingsPatch = Schema.Struct({
  enabled: Schema.optionalKey(Schema.Boolean),
  binaryPath: Schema.optionalKey(TrimmedString),
  modelOverride: Schema.optionalKey(TrimmedString),
  customModels: Schema.optionalKey(Schema.Array(Schema.String)),
});
